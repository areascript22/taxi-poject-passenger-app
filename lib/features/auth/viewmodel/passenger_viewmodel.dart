...
//Send OTP via wahtsapp
  ...
  Future<bool> verifyPhoneAndLogin(String phoneNumber) async {
    final phone = "+593$phoneNumber";
    logger.f("Phone number: ${phone}");
    try {
      final HttpsCallable callable = FirebaseFunctions.instance.httpsCallable('getAuthCustomToken');
...
//VErify OTP SMS or Whatsapp
  void verifyOtpSMSOrWhatsapp(String smsCode, BuildContext context) async {
    loading = true;
    if (smsCode.isEmpty) {
      ToastMessageUtil.showToast("Ingrese el código SMS para continuar",context);
      loading = false;
      return;
    }
...
//To verifiy code SMS
  Future<bool> _verifySms(String smsCode, BuildContext context) async {
    if (verificationId == null) {
      ...
      ToastMessageUtil.showToast("Código inválido. Inténtelo de nuevo.",context);
      return false;
    }
    ...
    try {
      ...
      if(context.mounted){
        ToastMessageUtil.showToast("Ocurrió un error inesperado. Inténtelo nuevamente.",context);
      }
      return false;
    }
  }
